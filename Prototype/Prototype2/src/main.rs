...
fn main() {
    App::new()
        ...
        .add_systems(Update, create_creature_attack.after(move_entities))
        .add_systems(Update, create_player_attack.before(move_entities))
...
fn create_creature_attack(
    mut query: Query<(&GridPos, &mut CreatureAttackDirection), With<Creature>>,
     mut commands: Commands,
asset_server: Res<AssetServer>,
            ) {
//BUG FIX: make it so you can aim off the screen
    let texture_handle_fire_attack = asset_server.load("fire_attack.png");
    for (GridPos, mut CreatureAttackDirection) in query.iter_mut() {
   let mut attack_x = GridPos.xgrid; 
   let mut attack_y = GridPos.ygrid; 
   match
